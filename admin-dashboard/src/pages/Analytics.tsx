...
import { ContentItem, Performer } from '../types'
...
export default function Analytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const views24h = allContent.reduce((sum: number, item: ContentItem) => sum + Math.floor(item.viewCount * 0.3), 0)
      const downloads24h = allContent.reduce((sum: number, item: ContentItem) => sum + Math.floor(item.downloadCount * 0.3), 0)
      ...
      const performerStats = allPerformers.map((performer: Performer) => {
        const performerContent = allContent.filter((c: ContentItem) => c.performerId === performer.id)
        const totalViews = performerContent.reduce((sum: number, c: ContentItem) => sum + c.viewCount, 0)
        const totalDownloads = performerContent.reduce((sum: number, c: ContentItem) => sum + c.downloadCount, 0)
        return {
          ...performer,
      ...
      }).sort((a: Performer, b: Performer) => (b.totalViews || 0) - (a.totalViews || 0)).slice(0, 5)
                ...
                >
                  ...
                  <Box sx={{ textAlign: 'right' }}>
                    <Typography variant="h6" color="primary">
                      {formatNumber(performer.totalViews || 0)}
                    </Typography>
                    <Typography variant="caption" color="text.secondary">
                      مشاهدة
                    </Typography>
