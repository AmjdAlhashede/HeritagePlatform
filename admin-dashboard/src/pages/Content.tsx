...
export default function Content() {
  ...
  const fetchContent = async () => {
    try {
      ...
      // جلب المحتوى من قائمة الرفع أيضاً
      const queueResponse = await api.get('/upload/queue')
      const queueContent = queueResponse.data || []
      
      // دمج المحتوى المنشور مع المحتوى في الانتظار
      const allContent = [
        ...publishedContent,
        ...queueContent.filter((item: any) => item.status === 'published')
      ]
            ...
            {filteredAndSortedContent.length === 0 ? (
              ...
              filteredAndSortedContent.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{formatDuration(item.duration || 0)}</TableCell>
